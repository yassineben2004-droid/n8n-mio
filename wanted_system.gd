...
const CAR_SPEED         := 7.5
const CAR_CHASE_DIST    := 60.0  # distanza max inseguimento auto
...
func _process(delta: float) -> void:
	...
	# Auto polizia inseguono il player
	_update_police_cars(delta)

	# Check arresto (NPC poliziotto a contatto)
...
func _update_police_cars(delta: float) -> void:
	...
	for car in _police_cars:
		...
		if dist > CAR_CHASE_DIST:
			continue
		# Muovi l'auto verso il player (visiva, no fisica)
...
func _arrest() -> void:
	...
	# Respawn posizione sicura dopo 2 secondi
