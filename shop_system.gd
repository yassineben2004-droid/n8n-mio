...
const ITEMS := [
	{"name": "Panino",      "price": 10, "hp": 25,  "emoji": "🥪"},
	{"name": "Birra",       "price": 5,  "hp": 10,  "emoji": "🍺"},
	{"name": "Caffè",       "price": 3,  "hp": 5,   "emoji": "☕"},
	{"name": "Pizza slice", "price": 15, "hp": 40,  "emoji": "🍕"},
]
...
func _build_menu() -> void:
	...
	for i in range(ITEMS.size()):
		...
		var row   := Label.new()
		row.name  = "Item%d" % i
		row.text  = "[%d] %s  +%dHP  €%d" % [i + 1, item["name"], item["hp"], item["price"]]
...
func _process(_delta: float) -> void:
	...
	if near and not _near_shop:
		_near_shop = true
		_prompt_label.text = "[ E ]  Entra al Caffè Molinari"
